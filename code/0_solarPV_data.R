...
for (s in 6:nrow(solar_map)) {
  ...
  gea <- solar_map$gea[s]
  site <- solar_map$site[s]
  
  epw <- read_epw(paste0(readfile_path, "weather/", filename))
  ...
  beta <- 15
  phi <- solar_map$latitude[s] # latitude
  ...
  for (i in 1:L) {
    ...
    sin_a[i] <- sin(radians(phi)) * sin(delta) + cos(radians(phi)) * cos(delta) * cos(omega)
    a[i] <- degrees(asin(sin_a[i]))
    
    A[i] <- degrees(atan(sin(omega) / (cos(radians(phi)) * tan(delta) - sin(radians(phi)) * cos(omega))))
    cos_theta[i] <- (sin(radians(phi)) * cos(radians(beta)) - cos(radians(phi)) * cos(radians(gamma)) * sin(radians(beta))) * sin(delta) + 
      (cos(radians(phi)) * cos(radians(beta)) + sin(radians(phi)) * cos(radians(gamma)) * sin(radians(beta))) * cos(delta) * cos(omega) + 
  ...
  #### EXPORT ####
  write_csv(df_solar, paste0(output_path, str_glue("{gea}_solar.csv")))
}
