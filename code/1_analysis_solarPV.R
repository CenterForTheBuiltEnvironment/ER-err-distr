...
output_path <- paste0(readfile_path, "results_solar/")
...
for (area in gea_map$gea_name){
  ...
  file_to_read <- all_files[grep(area, all_files)]
...
for (area in gea_map$gea_name){
  ...
  for (perc in seq(10, 40, by = 10)){
    ...
    ifelse(!dir.exists(file.path(str_glue(paste0(output_path, "{g}/{perc}/")))), dir.create(file.path(str_glue(paste0(output_path, "{g}/{perc}/")))), FALSE)
    ...
    write_rds(result, str_glue(paste0(output_path, "{g}/{perc}/", "hourly.rds")), compress = "gz")
...
#### ANALYSIS ####
readfile_path <- "../readfiles/results_solar/"
figs_path <- "../figs_solar/"
...
for (area in gea_map$gea_name){
  ...
  for (perc in seq(10, 40, by = 10)){
    ...
    ggsave(filename = str_glue("{area}_{perc}_summary.png"), path = figs_path, units = "in", height = 8, width = 18, dpi = 300)
